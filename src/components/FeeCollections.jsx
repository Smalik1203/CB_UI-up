...
export default function FeeCollections() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (uErr) throw uErr;
        setMe({ id: user.id, role: urec.role, school_code: urec.school_code });

        // Load classes
        const { data: classData, error: classErr } = await supabase
          .from("class_instances")
          .select("id, grade, section")
          .eq("school_code", urec.school_code)
        ...
        message.error(e.message || "Failed to initialize");
      } finally {
        setBoot(false);
      }
    })();
  ...
  const loadCollectionData = useCallback(async (cid, dateFilter = null) => {
    ...
    try {
      ...
      message.error(e.message || "Failed to load collection data");
    } finally {
      setLoading(false);
    }
  }, [me.school_code]);
  ...
  const handleClassChange = useCallback((cid) => {
    setClassId(cid);
    if (cid) {
      loadCollectionData(cid, dateRange);
    } else {
      setCollectionData([]);
    }
  }, [loadCollectionData, dateRange]);
  ...
  }, [classId, loadCollectionData]);

  // ---------- save payment ----------
  const savePayment = async (values) => {
    if (!paymentModal.student) return;
    if (!paymentModal.student.plan_id) {
      message.error("No fee plan found for this student. Please create a plan in Fee Management first.");
      return;
    }
    ...
    try {
      ...
      message.error(e.message || "Failed to record payment");
    } finally {
  ...
  const stats = useMemo(() => {
    if (!collectionData || collectionData.length === 0) {
      return { totalPlan: 0, totalCollected: 0, totalOutstanding: 0, avgCollection: 0 };
    }
    ...
    const avgCollection = totalPlan > 0 ? Math.round((totalCollected / totalPlan) * 100) : 0;

    return { totalPlan, totalCollected, totalOutstanding, avgCollection };
  }, [collectionData]);
